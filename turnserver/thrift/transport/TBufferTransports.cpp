...
uint32_t TBufferedTransport::readSlow(uint8_t* buf, uint32_t len) {
  ...
  setReadBuffer(rBuf_.data(), transport_->read(rBuf_.data(), rBufSize_));
...
void TBufferedTransport::writeSlow(const uint8_t* buf, uint32_t len) {
  uint32_t have_bytes = static_cast<uint32_t>(wBase_ - wBuf_.data());
  ...
  if ((have_bytes + len >= 2*wBufSize_) || (have_bytes == 0)) {
    // TODO(dreiss): writev
    if (have_bytes > 0) {
      transport_->write(wBuf_.data(), have_bytes);
    }
  ...
  buf += space;
  len -= space;
  transport_->write(wBuf_.data(), wBufSize_);

  // Copy the rest into our buffer.
  assert(len < wBufSize_);
  memcpy(wBuf_.data(), buf, len);
  wBase_ = wBuf_.data() + len;
  return;
}
...
bool TFramedTransport::readFrame() {
  ...
  while (size_bytes_read < sizeof(sz)) {
    ...
    if (bytes_read == 0) {
      if (size_bytes_read == 0) {
        ...
        throw TTransportException(TTransportException::END_OF_FILE, "No more data to read after partial frame header.");
#endif
		assert(0);
      }
    }
  ...
  transport_->readAll(rBuf_.data(), sz);
  setReadBuffer(rBuf_.data(), sz);
  return true;
}

void TFramedTransport::writeSlow(const uint8_t* buf, uint32_t len) {
  // Double buffer size until sufficient.
  uint32_t have = static_cast<uint32_t>(wBase_ - wBuf_.data());
  uint32_t new_size = wBufSize_;
  if (len + have < have /* overflow */ || len + have > 0x7fffffff) {
#if AGORARTC_HAS_EXCEPTION
	  throw TTransportException(TTransportException::BAD_ARGS, "Attempted to write over 2 GB to TFramedTransport.");
#endif
	assert(0);
  }
  ...
  memcpy(new_buf.data(), wBuf_.data(), have);

  // Now point buf to the new one.
  wBuf_.swap(new_buf);
  wBufSize_ = new_size;
  wBase_ = wBuf_.data() + have;
  wBound_ = wBuf_.data() + wBufSize_;
...
void TFramedTransport::flush()  {
  ...
  sz_hbo = static_cast<uint32_t>(wBase_ - (wBuf_.data() + sizeof(sz_nbo)));
  sz_nbo = (int32_t)htonl((uint32_t)(sz_hbo));
  memcpy(wBuf_.data(), (uint8_t*)&sz_nbo, sizeof(sz_nbo));
  ...
  if (sz_hbo > 0) {
    ...
    // up an exception
    wBase_ = wBuf_.data() + sizeof(sz_nbo);
  ...
  if (wBufSize_ > bufReclaimThresh_) {
    ...
    wBuf_.resize(wBufSize_);
    setWriteBuffer(wBuf_.data(), wBufSize_);

    // reset wBase_ with a pad for the frame size
    int32_t pad = 0;
    wBase_ = wBuf_.data() + sizeof(pad);
  }
}

uint32_t TFramedTransport::writeEnd() {
  return static_cast<uint32_t>(wBase_ - wBuf_.data());
}
...
uint32_t TFramedTransport::readEnd() {
  ...
  uint32_t bytes_read = static_cast<uint32_t>(rBound_ - rBuf_.data() + sizeof(uint32_t));

  if (rBufSize_ > bufReclaimThresh_) {
      rBufSize_ = 0;
      rBuf_.resize(0);
      setReadBuffer(rBuf_.data(), rBufSize_);
  }
...
void TMemoryBuffer::ensureCanWrite(uint32_t len) {
  ...
  void* new_buffer = ::realloc(buffer_, new_size);
  if (new_buffer == NULL) {
#if AGORARTC_HAS_EXCEPTION
	  throw std::bad_alloc();
#endif
