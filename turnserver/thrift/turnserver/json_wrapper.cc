...
void JsonWrapper::setStringValue(const std::string& name, const std::string& value)
{
...
bool JsonWrapper::isBoolean() const
{
	if (m_root)
	{
		if (m_root->type == cJSON_True || m_root->type == cJSON_False)
			return true;
	}
	return false;
}
...
{
	return doToString(m_root, formatted);
}
...
void JsonWrapper::doMerge(cJSON* root1, cJSON* root2)
{
	...
	for (cJSON* item2 = root2->child; item2 != NULL; item2 = item2->next)
	{
		if (!item2->string)
